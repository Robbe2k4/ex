...
switch (_killType) do 
{
    ...
    _weaponScope = ""; 
		...
		_systemChat = format ["%1 %2 by %3 with a %4 from %5 meters away!", name _victim, _npcDeath, name _killer, _weaponDisplayName,  floor(_victim distance _killer)];
    ...
    _weaponScope = ""; 
		...
		else 
		{
			...
			forEach _perks;
			_systemChat = format ["%1 was killed by %2 with a %3 from %4 meters away! (%5)", name _victim, name _killingPlayer, _weaponDisplayName, floor(_victim distance _killer), _perkNames joinString ", "];
			}
			else
			{
			_perkNames = [];
...
if (_killType isEqualTo 7) then
{
	"playerkill" remoteExec ["playSound"];
};

if (_killType isEqualTo 6) then
{
	"bambikill" remoteExec ["playSound"];
};

if (_killType isEqualTo 5) then
{
	"teamkill" remoteExec ["playSound"];
};
