...
if (_keyCode in (actionKeys "User13")) exitWith
{
	if !(ExileClientIsHandcuffed || ExileIsPlayingRussianRoulette) then
	{
		...
		else
		{
			...
			{
				["InfoTitleAndText", ["Too Late!", "You're already dead!"]] call ExileClient_gui_toaster_addTemplateToast;
			}
			else
			{
				if !("Exile_Item_InstaDoc" in (magazines player)) then
				{
					["InfoTitleAndText", ["Uh Oh!", "You'd lost your InstaDoc somewhere!"]] call ExileClient_gui_toaster_addTemplateToast;
				}
				else
				{
					if ("Exile_Item_InstaDoc" in (magazines player)) then
	...
	if !(ExileClientIsHandcuffed || ExileIsPlayingRussianRoulette) then
	{
		...
		else
		{
			...
			else
			{
				if !("Exile_Item_InstaDoc" in (magazines player)) then
				{
					["InfoTitleAndText", ["Uh Oh!", "You'd lost your Bandages somewhere!"]] call ExileClient_gui_toaster_addTemplateToast;
				}
				else
				{
