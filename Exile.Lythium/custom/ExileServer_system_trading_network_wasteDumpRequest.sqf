...
private["_sessionID", "_parameters", "_vehicleNetID", "_mode", "_vehicleObject", "_vehicleDBID", "_playerObject", "_cargo", "_revenue", "_playerMoney", "_respectGain", "_playerRespect", "_logging", "_traderLog", "_responseCode", "_crateList", "_vehicleName"];
...
_crateList = ["Supply Box", "Basic Weapons [NATO]", "Basic Ammo [NATO]", "Cargo Net [AAF]", "Cargo Net [CSAT]", "Cargo Net [NATO]", "Cargo Net (Box)"];
_vehicleName = getText(configFile >> "CfgVehicles" >> (typeOf _vehicleObject) >> "displayName");
if ((_mode isEqualTo 1) && (_vehicleName in _crateList)) then
 {
	["ErrorTitleOnly", ["Next time sell the Cargo AND the Crate!"]] call ExileClient_gui_toaster_addTemplateToast;
	deleteVehicle _vehicleObject;
};
