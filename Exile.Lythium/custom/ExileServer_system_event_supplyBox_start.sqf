...
["toastRequest", ["InfoTitleAndText", ["Care Package Incoming!", "The UNWTO is going to drop a care package in 10 minutes. Check your map for the location."]]] call ExileServer_system_network_send_broadcast;
_marker = createMarker [ format["ExileSupplyBox%1", diag_tickTime], _position];
_marker setMarkerType "ExileHeart";
uiSleep (60 * 5);
["toastRequest", ["InfoTitleAndText", ["Care Package Incoming!", "The UNWTO is going to drop a care package in 5 minutes. Check your map for the location."]]] call ExileServer_system_network_send_broadcast;
uiSleep (60 * 4);
["toastRequest", ["InfoTitleAndText", ["Care Package Incoming!", "The UNWTO is going to drop a care package in 1 minute. Check your map for the location."]]] call ExileServer_system_network_send_broadcast;
uiSleep (60 * 1);
_box = createVehicle ["Exile_Container_SupplyBox", [0, 0, 0], [], 0, "CAN_COLLIDE"];
_box setPosATL _position;
_box setDir (random 360);
if (_boxType isEqualTo "Money") then
	{
	_box setVariable ["ExileMoney", 35000,true];
	};
