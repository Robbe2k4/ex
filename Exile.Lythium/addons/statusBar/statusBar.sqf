...
//	systemChat format["StatusBar Initialized", _rscLayer];
	...
	while {true} do 
	{
		...
		_deaths = if(ExileClientPlayerDeaths == 0)then{0.001}else{ExileClientPlayerDeaths};
		_kdratio = [_kills / _deaths, 2] call ExileClient_util_math_round;
		...
		switch(_minutes) do
	{
		...
		//Cardinal Directions
		_cardinal = _cardinalDefault;
		if((_dir >= 337.5) && (_dir < 22.5 )) then {_cardinal = "N";};
		if((_dir >= 22.5) && (_dir < 67.5 )) then {_cardinal = "NE";};
		if((_dir >= 67.5) && (_dir < 112.5 )) then {_cardinal = "E";};
		if((_dir >= 112.5) && (_dir < 157.5 )) then {_cardinal = "SE";};
		if((_dir >= 157.5) && (_dir < 202.5 )) then {_cardinal = "S";};
		if((_dir >= 202.5) && (_dir < 247.5 )) then {_cardinal = "SW";};
		if((_dir >= 247.5) && (_dir < 292.5 )) then {_cardinal = "W";};
		if((_dir >= 292.5) && (_dir < 337.5 )) then {_cardinal = "NW";};
		...
		((uiNamespace getVariable "osefStatusBarAdmin")displayCtrl 55554)ctrlSetStructuredText parseText 
			...
			<t shadow='1' shadowColor='#000000' color='%10'><img size='1.0'  shadowColor='#000000' image='addons\statusbar\icons\kdratio.paa' color='%10'/> %24</t>
			...
			<t shadow='1' shadowColor='#000000' color='%15'><img size='1.0'  shadowColor='#000000' image='addons\statusbar\icons\compass.paa' color='%10'/>%23 %17</t>
